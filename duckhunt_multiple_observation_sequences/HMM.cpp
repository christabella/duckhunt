...
namespace ducks {
    ...
    HMM::HMM() : 
        ...
        A_num(N, std::vector<double>(N, 0)),
        A_denom(N, std::vector<double>(N, 0)),
        A_num_temp(N, std::vector<double>(N, 0)),
        A_denom_temp(N, std::vector<double>(N, 0)),
        B(N, std::vector<double>(M, 0)),
        B_num(N, std::vector<double>(M, 0)),
        B_denom(N, std::vector<double>(M, 0)),
        B_num_temp(N, std::vector<double>(M, 0)),
        B_denom_temp(N, std::vector<double>(M, 0)),
        pi(N, 0),
        pi_num(N, 0),
        pi_denom(N, 0),
        pi_num_temp(N, 0),
    ...
    void HMM::estimateModel(const std::vector<EMovement> &Om) {
        ...
        while (iters < maxIters && logProb > oldLogProb + 0.0000001) {
            ...
            double P_O = 0; // Probability of Observation Sequence P(O|lambda)
            for (int i = 0; i < N; ++i) 
                P_O += alpha[T-1][i];
            // std::cerr << "FAILING\n";
            ...
            // Re-estimate pi
            for (int i = 0; i < N; ++i){
                pi_num_temp[i] = pi_num[i] + P_O * gamma[0][i];
                pi_denom_temp[i] = pi_denom[i] + P_O;
                pi[i] = pi_num_temp[i] / pi_denom_temp[i];
            }
            ...
            for (int i = 0; i < N; ++i) {
                for (int j = 0; j < N; ++j) {
                    ...
                    A_num_temp[i][j] = A_num[i][j] + P_O * numer;
                    A_denom_temp[i][j] = A_denom[i][j] + P_O * denom;
                    A[i][j] = A_num_temp[i][j] / A_denom_temp[i][j];
                }
            }
            ...
            for (int i = 0; i < N; ++i) {
                for (int j = 0; j < M; ++j) {
                    ...
                    B_num_temp[i][j] = B_num[i][j] + P_O * numer;
                    B_denom_temp[i][j] = B_denom[i][j] + P_O * denom;
                    B[i][j] = B_num_temp[i][j] / B_denom_temp[i][j];                }
            }
        ...
        for (int i = 0; i < N; ++i) {
            pi_num[i] = pi_num_temp[i];
            pi_denom[i] = pi_denom_temp[i];
        }

        for (int i = 0; i < N; ++i) {
            for (int j = 0; j < N; ++j) {
                A_num[i][j] = A_num_temp[i][j];
                A_denom[i][j] = A_denom_temp[i][j];
            }
        }

        for (int i = 0; i < N; ++i) {
            for (int j = 0; j < M; ++j) {
                B_num[i][j] = B_num_temp[i][j];
                B_denom[i][j] = B_denom_temp[i][j];
            }
        }
        // std::cerr << "----------A----------\n";
        // printMatrix(A);
